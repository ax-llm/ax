...
| Child MCP execution | `owned-child-mcp-inheritance-{all,none,allowlist,empty,child-config}.json`; TypeScript `src/ax/mcp/execution.test.ts` | Actual parent and child forwards, selected MCP requests, denied namespaces, parent continuation, child configuration precedence, and actor request filtering in all five targets |
