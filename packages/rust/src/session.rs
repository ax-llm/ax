...
pub(crate) fn worker_control(
    parent: Option<AxRunControl>,
    relay: impl Fn(Value) + Send + Sync + 'static,
) -> AxRunControl {
    AxRunControl(Arc::new(ControlState {
        parent,
        relay: Some(Arc::new(relay)),
        ..ControlState::default()
    }))
}
pub(crate) fn emit_worker_event(control: &AxRunControl, event: Value) {
    control.emit(event)
}
...
impl AxRunControl {
    ...
    fn pending(&self, path: &str, after: usize) -> AxResult<(Vec<Value>, usize)> {
        if let Some(parent) = &self.0.parent {
            return parent.pending(path, after);
        }
...
pub type AxSessionWebSocketFactory =
    Arc<dyn Fn(&str, &Value) -> AxResult<Arc<dyn AxSessionSocket>> + Send + Sync>;
...
impl AxSessionSocket for NativeSessionSocket {
    fn send(&self, event: Value) -> AxResult<()> {
        if self.closed.load(Ordering::SeqCst) {
            return Err(AxError::runtime("Session socket closed"));
        }
        self.socket
            .lock()
            .map_err(|_| AxError::runtime("Session socket lock poisoned"))?
            .send(&event)
    }
    fn recv(&self, timeout: Duration) -> AxResult<Option<Value>> {
        if self.closed.load(Ordering::SeqCst) {
        ...
        let mut socket = self
            ...
            .lock()
            .map_err(|_| AxError::runtime("Session socket lock poisoned"))?;
        self.connection.set_read_timeout(Some(
            timeout
        ...
        match socket.socket.read() {
            ...
            Err(error) => Err(AxError::runtime(format!(
                "Responses socket failed; work was not replayed: {error}"
            ))),
        }
    }
    fn close(&self) {
...
#[cfg(feature = "realtime")]
pub(crate) fn native_session_socket(
    url: &str,
    headers: &Value,
) -> AxResult<Arc<dyn AxSessionSocket>> {
    let headers = headers
        .as_object()
        .into_iter()
        .flatten()
    ...
    Ok(Arc::new(NativeSessionSocket {
        ...
        match tokio::time::timeout(Duration::from_millis(10), future.as_mut()).await {
            Ok(result) => {
                return result.map(Some).map_err(|error| {
                    ...
                    let mut error = AxError::from(error);
                    if timeout {
                        error.error_type = Some("AxAIServiceTimeoutError".into());
                        error.message = format!(
                            "Responses HTTP request timed out; work was not replayed: {}",
                            error.message
                        );
                    }
                    error
...
fn session_http_stream(
    ...
    runtime.block_on(async {
        ...
        if status >= 400 {
            let Some(bytes) = session_http_wait(response.bytes(), cancelled).await? else {
                return Ok(());
            };
            ...
            return Err(AxError::runtime(format!(
                "Session HTTP request failed: {status}"
            )));
        }
        ...
        while let Some(chunk) = session_http_wait(response.chunk(), cancelled).await? {
            ...
            for byte in chunk {
                if decoder.skip_lf {
                    decoder.skip_lf = false;
                    if byte == b'\n' {
                        continue;
                    }
                }
                let event = if byte == b'\r' || byte == b'\n' {
                    decoder.skip_lf = byte == b'\r';
                    decoder.process_line()?
                } else {
                ...
                if let Some(event) = event {
                    ...
                    if sender.send(Ok(event)).is_err() || completed {
                        return Ok(());
                    }
                }
                if decoder.done {
...
impl ResponsesSession {
    pub(crate) fn open(
        ...
        if let Some(socket) = session.socket.clone() {
            ...
            let cursor = session.transport_cursor.clone();
            std::thread::spawn(move || {
                let result =
                    std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> AxResult<()> {
                        while !cancelled.load(Ordering::SeqCst) {
                            if let Some(event) = socket.recv(Duration::from_millis(10))? {
                                {
                                    let mut state = cursor
                                        .lock()
                                        .map_err(|_| AxError::runtime("Session cursor poisoned"))?;
                                    *state =
                                        core_value_to_json(&openai_responses_transport_cursor(&[
                    ...
                    .unwrap_or_else(|_| {
                        Err(AxError::runtime(
                            "Session socket panicked; work was not replayed",
                        ))
                    });
    ...
    fn send(&mut self, payload: Value) -> AxResult<()> {
        ...
        if self.cancelled.load(Ordering::SeqCst) {
            return Err(AxError::runtime("Session closed"));
        }
            ...
            let result =
                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> AxResult<()> {
                    if transport.is_none() {
                        return session_http_stream(call, &cancelled, &sender);
                    }
            ...
            match self.receiver.recv_timeout(timeout) {
                Ok(event) => {
                    ...
                    if event["type"] == "response.created" {
                        self.previous = event["response"]["id"].as_str().map(str::to_string);
                    }
    ...
    fn update(&mut self, update: &Value) -> AxResult<&'static str> {
        {
            ...
            if update["type"] == "steer" && !cursor["active_id"].is_null() {
                if let Some(socket) = &self.socket {
                    socket.send(json!({"type":"response.steer","previous_response_id":cursor["active_id"],"input":[{"role":"user","content":[{"type":"input_text","text":update["text"]}]}]}))?;
                    return Ok("native");
                }
            }
        }
...
pub(crate) fn dispatch_run_route(
    ...
    fn resolve<'a>(
        mut client: &'a mut dyn AxAIClient,
        routes: &[Value],
    ) -> AxResult<&'a mut dyn AxAIClient> {
        for route in routes {
            client = client.pinned_chat_run_client(
                route
                    .as_str()
                    .ok_or_else(|| AxError::validation("Invalid run route"))?,
            )?;
        }
        Ok(client)
    }
    let routes = envelope["route"].as_array().cloned().unwrap_or_default();
    let client = resolve(client, &routes)?;
    let request = envelope["request"].clone();
    match method {
        "route_owned_worker" => Ok(publish_owned_client_factory(client.owned_worker_factory())),
        "route_select" => Ok(client
            .pin_chat_run(&request, &options)?
            .map(Value::String)
            .unwrap_or(Value::Null)),
        "route_features" => Ok(client.get_features(request.as_str())),
        "route_preprocess" => client.preprocess_pinned_chat_run(
            request["selected"]
                .as_str()
                .ok_or_else(|| AxError::validation("Invalid run route"))?,
            request["request"].clone(),
        ),
        "route_open_session" => Ok(publish_open_session(
            client.open_chat_session(request, options)?,
        )),
        "route_observe_session" => {
...
fn pinned_run_client<'a>(
    client: &'a mut dyn AxAIClient,
    request: &mut Value,
    options: &Value,
    ...
    if depth >= 32 {
        return Err(AxError::runtime("Cyclic run routing"));
    }
    let route = if selected {
        routes.get(depth).cloned()
    } else {
        let route = client.pin_chat_run(request, options)?;
    ...
    if let Some(route) = route {
        *request = client.preprocess_pinned_chat_run(&route, std::mem::take(request))?;
        pinned_run_client(
            client.pinned_chat_run_client(&route)?,
            request,
            options,
            routes,
            depth + 1,
...
impl SessionRun {
    ...
    fn start(&mut self, call: Value) -> AxResult<()> {
        let call = core_value_to_json(&chat_session_normalize_call(&[core_value_from_json(
            &call,
        )])?);
        ...
        loop {
            ...
            if let Some(control) = &self.control {
                ...
                for update in updates {
                    ...
                    if self.session.as_mut().unwrap().update(&update)? == "native" {
                        chat_session_native_update(&[
                            self.state.clone(),
                            core_value_from_json(&update["id"]),
                        ])?;
                    } else {
                        self.applied
                            .push(update["id"].as_str().unwrap_or("").to_string());
                    }
                }
            }
            while let Ok(delivery) = self.results.try_recv() {
                ...
                if !core_truthy(&chat_session_record_result(&[
                    self.gen.clone(),
                    self.state.clone(),
                    core_value_from_json(&delivery.call),
                ...
                .next(Duration::from_millis(10))?
            {
                ...
                if event["type"] == "steering" {
                    let result = core_value_to_json(&chat_session_native_event(&[
                        self.state.clone(),
                        core_value_from_json(&event),
                    ])?);
                    if !result["applied_id"].is_null() {
                        self.emit(
                            "applied",
                            json!({"update_id":result["applied_id"],"timing":"native"}),
                        );
                    }
                }
                    ...
                    ])?)
                {
                    ...
                    let completion = chat_session_completion(&[
            ...
            match action["type"].as_str().unwrap_or("") {
                ...
                "validate" => {
                    if let Some(response) = &self.last {
                        return Ok(core_value_to_json(&chat_session_result(&[
                            core_value_from_json(response),
                            core_value_from_json(&core_value_to_json(&self.state)["response_id"]),
                        ])?));
                    }
                }
                _ => {}
    ...
    pub(crate) fn finish(&mut self, error: Option<&AxError>) {
        ...
        let pending = if let Some(session) = &mut self.session {
            ...
            chat_session_close_state(&[self.state.clone()])
...
#[cfg(test)]
mod tests {
    struct OwnedTestProgram {
        gen: AxGen,
        owner_state: Rc<RefCell<usize>>,
    }
    impl AxProgram for OwnedTestProgram {
    ...
    impl AxExecutableProgram for OwnedTestProgram {
        ...
        fn owned_worker_factory(&self) -> Option<AxOwnedProgramFactory> {
            let create = self.gen.owned_worker_factory()?;
            let count = *self.owner_state.borrow();
            Some(Box::new(move || {
                Box::new(Self {
                    gen: create(),
                    owner_state: Rc::new(RefCell::new(count)),
                })
            }))
        }
        fn get_chat_log(&self) -> Vec<Value> {
            self.gen.chat_log.clone()
        }
        fn get_traces(&self) -> Vec<Value> {
            self.gen.traces.clone()
        }
    }
    #[test]
    fn owned_flow_workers_overlap_http() -> AxResult<()> {
        ...
        let server = std::thread::spawn(move || {
            ...
            assert_eq!(workers.len(), 2, "Both nodes must start");
            for worker in workers {
                worker.join().unwrap().unwrap();
            }
        });
        let release = std::thread::spawn(move || {
            let first = arrived_rx.recv_timeout(Duration::from_secs(3));
            let second = arrived_rx.recv_timeout(Duration::from_secs(3));
            *gate.0.lock().unwrap() = true;
            gate.1.notify_all();
            assert!(
                first.is_ok() && second.is_ok(),
                "Parallel barrier not reached"
            );
        });
        let mut client = ai(
            "openai",
            json!({"api_key":"worker-test","model":"gpt-5.6","api_url":endpoint}),
        )?;
        ...
        let nested = flow("nested")
            .execute("inner", ax("question -> answer")?)
            .returns(json!({"answer":"innerResult.answer"}));
        let custom = OwnedTestProgram {
            gen: ax("question -> answer")?,
            owner_state: Rc::new(RefCell::new(0)),
        };
        let mut workflow = flow("overlap")
            .execute_program(
                "first",
                nested,
                &json!({"reads":["question"],"writes":["firstResult"],"isBarrier":false}),
            )
            .execute_program(
                "second",
                custom,
                &json!({"reads":["question"],"writes":["secondResult"],"isBarrier":false}),
            )
            .returns(json!({"first":"firstResult","second":"secondResult"}));
        let result = workflow.forward_with_options(
            &mut client,
            json!({"question":"Ready"}),
            json!({"stream":false,"model":"smart"}),
        );
        release.join().unwrap();
        server.join().unwrap();
        assert_eq!(
            result?,
            json!({"first":{"answer":"DONE"},"second":{"answer":"DONE"}})
        );
        assert_eq!(
            core_iter(&core_get(
                &workflow.state,
    ...
    impl AxTransport for FailureTransport {
        fn owned_worker_factory(&self) -> Option<AxOwnedTransportFactory> {
            ...
            Some(Box::new(move || Box::new(FailureTransport(gate))))
        }
        fn send(&mut self, request: Value) -> AxResult<Value> {
            ...
            let (guard, timeout) = gate
                .all
                .wait_timeout_while(lock, Duration::from_secs(3), |_| {
                    gate.started.load(Ordering::SeqCst) < 3
                })
                .unwrap();
            drop(guard);
            if timeout.timed_out() {
                return Err(AxError::runtime("Independent nodes did not overlap"));
            }
            let content = if body.contains("lateAnswer") {
    ...
    fn owned_flow_failure_discards_late_work() -> AxResult<()> {
        ...
        struct Release(Arc<FailureGate>);
        ...
        control.on_event(move |event| {
            if event["type"] == "completed" && event["path"] == "root/fast" {
                observed.fast.store(true, Ordering::SeqCst);
            }
        });
        let mut client = ai("openai", json!({"api_key":"test","model":"gpt-5.6"}))?
            .with_transport(FailureTransport(gate.clone()));
        let mut workflow = flow("failure")
            .execute("fast", ax("question -> fastAnswer")?)
            .execute("fail", ax("question -> failAnswer")?)
            .execute("late", ax("question -> lateAnswer")?);
        let started = Instant::now();
        let error = workflow
            .forward_with_options(
                &mut client,
                json!({"question":"Ready"}),
                AxForwardOptions::from(
                    json!({"stream":false,"maxSteps":1,"validationRetries":0,"infraRetries":0}),
                )
        ...
        assert!(!gate.late.load(Ordering::SeqCst));
        let state = core_value_to_json(&core_get(
            &workflow.state,
            &CoreValue::from("completed_state"),
            CoreValue::Null,
        ));
        assert_eq!(state["fastResult"], json!({"fastAnswer":"DONE"}));
        gate.release.store(true, Ordering::SeqCst);
        let started = Instant::now();
        while !gate.late.load(Ordering::SeqCst) && started.elapsed() < Duration::from_secs(3) {
            std::thread::sleep(Duration::from_millis(1));
        }
        assert!(gate.late.load(Ordering::SeqCst));
        assert_eq!(
    ...
    struct InvalidArgumentsTransport {
        requests: Arc<AtomicUsize>,
        exhausted: bool,
        arguments: String,
    }
    impl AxTransport for InvalidArgumentsTransport {
        ...
        fn stream(&mut self, request: Value) -> AxResult<AxTransportStream> {
            let n = self.requests.fetch_add(1, Ordering::SeqCst) + 1;
            let event = if n == 1 {
                json!({"type":"response.completed","response":{"id":"invalid","model":"gpt-6-astra","output":[{"type":"function_call","id":"invalid-item","call_id":"invalid-call","name":"validated_lookup","arguments":self.arguments}]}})
            } else {
                assert!(!self.exhausted && n == 2, "Work replayed after exhaustion");
                let body = &request["json"];
                let outputs = body["input"].as_array().unwrap();
                assert_eq!(body["previous_response_id"], "invalid");
                assert_eq!(outputs.len(), 1);
                assert_eq!(outputs[0]["call_id"], "invalid-call");
    ...
    fn invalid_arguments_correction_and_step_exhaustion() -> AxResult<()> {
        for exhausted in [false, true] {
            for arguments in ["{}", r#"{"query":"ab"}"#] {
                let requests = Arc::new(AtomicUsize::new(0));
                let calls = Arc::new(AtomicUsize::new(0));
                let called = calls.clone();
                let mut client = ai("openai", json!({"api_key":"test","model":"gpt-6-astra"}))?
                    .with_transport(InvalidArgumentsTransport {
                        requests: requests.clone(),
                        exhausted,
                ...
                let result = program.forward_with_options(
                    ...
                    json!({"maxSteps":if exhausted{1}else{3}}),
                );
                if exhausted {
                    assert!(result.unwrap_err().message.contains("steps"));
                } else {
                    assert_eq!(result?, json!({"answer":"CORRECTED"}));
                }
        ...
        fn stream(&mut self, request: Value) -> AxResult<AxTransportStream> {
            ...
            let event = if n % 2 == 1 {
                assert!(
                    ...
                    "node inherited another conversation"
                );
                ...
                assert_eq!(
                    ...
                    "root update must reach future nodes"
                );
                assert_eq!(
                    body["input"][1],
    ...
    fn flow_conversation_isolation_and_future_root_updates() -> AxResult<()> {
        ...
        control.on_event(move |event| {
            if event["type"] == "applied" {
                observed.lock().unwrap().push(event["path"].clone());
            }
        });
        control.steer("Keep the reference exact.")?;
        let mut client = ai("openai", json!({"api_key":"test","model":"gpt-6-astra"}))?
            .with_transport(FlowTransport(requests.clone()));
        ...
        let mut workflow = flow("session-flow")
            .execute("first", ax("question -> answer")?.with_tool(lookup.clone()))
            .execute("second", ax("question -> answer")?.with_tool(lookup))
            .returns(json!({"first":"firstResult","second":"secondResult"}));
        for _ in 0..2 {
            let result = workflow.forward_with_options(
                &mut client,
                json!({"question":"Find reference"}),
                AxForwardOptions::from(json!({"model":"smart"})).with_control(control.clone()),
            )?;
            assert_eq!(
                result,
                json!({"first":{"answer":"REF-42"},"second":{"answer":"REF-42"}})
            );
        }
    ...
    impl AxSessionSocket for SteeringSocket {
        fn send(&self, event: Value) -> AxResult<()> {
            ...
            if self.pending {
                ...
                queue.push_back(json!({"type":"response.output_item.done","item":{"type":"function_call","id":"item","call_id":"pending-call","name":"lookup","arguments":"{}"}}));
            ...
            if event["type"] == "response.create" {
                ...
                assert_eq!(event["previous_response_id"], "parent");
        ...
        fn recv(&self, timeout: Duration) -> AxResult<Option<Value>> {
            ...
            let (mut queue, _) = self
                ...
                .unwrap();
            if self.closed.load(Ordering::SeqCst) {
                return Err(AxError::runtime("Socket closed"));
            }
            Ok(queue.pop_front())
        }
        fn close(&self) {
    ...
    fn native_websocket_transport_steering_and_close() -> AxResult<()> {
        ...
        let server = std::thread::spawn(move || {
            ...
            assert_eq!(steer["previous_response_id"], "parent");
            for event in [
            ...
            let closed = match socket.read() {
                Ok(tungstenite::Message::Close(_)) => true,
                Err(tungstenite::Error::Io(error)) => !matches!(
                    error.kind(),
        ...
        let mut client = ai(
            "openai",
            json!({"api_key":"test","model":"gpt-6-astra","base_url":endpoint}),
        )?
        .with_native_session_web_socket();
        assert_eq!(client.get_features(None)["nativeSteering"], true);
        let control = run_control();
        let weak = Arc::downgrade(&control.0);
        let steered = AtomicBool::new(false);
        control.on_event(move |event| {
            if event["type"] == "model.output" && !steered.swap(true, Ordering::SeqCst) {
                AxRunControl(weak.upgrade().unwrap())
                    .steer("Use CORRECTED.")
                    .unwrap();
            }
        });
        ...
        assert_eq!(
            program.forward_with_options(
                &mut client,
                json!({"question":"Find answer"}),
                AxForwardOptions::from(json!({})).with_control(control)
            )?,
            json!({"answer":"CORRECTED"})
        );
        assert_eq!(
            program
                .get_chat_log()
                .iter()
                .map(|v| v["remote_id"].clone())
                .collect::<Vec<_>>(),
            vec![json!("parent"), json!("successor")]
        );
        assert!(closed_rx
            .recv_timeout(Duration::from_secs(4))
            .expect("native socket was not closed"));
        server.join().unwrap();
        Ok(())
    }
    ...
    fn buffered_terminal_queues_steering() -> AxResult<()> {
        ...
        impl AxSessionSocket for BoundarySocket {
            ...
            fn recv(&self, timeout: Duration) -> AxResult<Option<Value>> {
                if self.reads.load(Ordering::SeqCst) == 3 {
        ...
        let (sender, receiver) = mpsc::channel();
        let socket = Arc::new(BoundarySocket {
        ...
        let configured = socket.clone();
        let mut client = ai("openai", json!({"api_key":"test","model":"gpt-6-astra"}))?
            .with_session_web_socket_factory(move |_, _| Ok(configured.clone()));
        let mut session = client
            .open_chat_session(
                json!({"chat_prompt":[{"role":"user","content":"probe"}]}),
                json!({}),
            )?
            .unwrap();
        receiver
            .recv_timeout(Duration::from_secs(2))
            .expect("Terminal frame was not received");
        assert_eq!(
            session.next(Duration::from_secs(2))?.unwrap()["type"],
            "response"
        );
        assert_eq!(
            session.update(&json!({"type":"steer","text":"Correct answer"}))?,
            "next-response"
        );
        assert_eq!(socket.sent.load(Ordering::SeqCst), 1);
        session.close();
        Ok(())
    }
    ...
    fn native_steering_successor_and_accounting() -> AxResult<()> {
        let socket = Arc::new(SteeringSocket::default());
        let configured = socket.clone();
        let mut client = ai(
            "openai",
            json!({"api_key":"test","model":"gpt-6-astra","base_url":"https://example.test/v1"}),
        )?
        .with_session_web_socket_factory(move |url, headers| {
            assert_eq!(url, "wss://example.test/v1/responses");
        ...
        control.on_event(move |event| {
            ...
            if event["type"] == "applied" {
                observed.lock().unwrap().push(event["timing"].clone());
            }
        });
        let mut program = ax("question -> answer")?;
        let result = program.forward_with_options(
        ...
        assert_eq!(result, json!({"answer":"CORRECTED"}));
        assert_eq!(*applied.lock().unwrap(), vec![json!("native")]);
        assert_eq!(socket.sent.lock().unwrap().len(), 2);
    ...
    fn cancellation_context_and_late_delivery() -> AxResult<()> {
        let socket = Arc::new(SteeringSocket {
            pending: true,
            ..SteeringSocket::default()
        });
        let configured = socket.clone();
        let mut client = ai("openai", json!({"api_key":"test","model":"gpt-6-astra"}))?
            .with_session_web_socket_factory(move |_, _| Ok(configured.clone()));
        ...
        let lookup = tool("lookup")
            .description("Lookup")
            .execution("background")
            .context_handler(move |_, context| {
                assert_eq!(context.call_id.as_deref(), Some("pending-call"));
                abort.abort();
                let start = Instant::now();
                while !context.is_cancelled() && start.elapsed() < Duration::from_secs(2) {
                    std::thread::sleep(Duration::from_millis(1));
                }
        ...
        let error = program
            ...
            .unwrap_err();
        assert!(error.to_string().contains("pending-call"), "{error}");
        assert!(start.elapsed() < Duration::from_secs(2));
        let start = Instant::now();
        while !settled.load(Ordering::SeqCst) && start.elapsed() < Duration::from_secs(2) {
            std::thread::sleep(Duration::from_millis(1));
        }
    ...
    fn noncooperative_cancellation_keeps_late_work_owned() -> AxResult<()> {
        ...
        let lookup = tool("lookup")
            ...
            .context_handler(move |_, context| {
                ...
                wait.lock()
                    ...
                    .expect("caller waited for noncooperative work");
                finished.store(true, Ordering::SeqCst);
                Ok(json!("LATE"))
            });
        let mut program = ax("question -> answer")?.with_tool(lookup);
        ...
        assert!(!settled.load(Ordering::SeqCst));
        assert!(socket.closed.load(Ordering::SeqCst));
        let traces = program.function_call_traces.clone();
        assert_eq!(traces.len(), 1);
        assert_eq!(traces[0]["id"], "pending-call");
        assert_eq!(traces[0]["status"], "unresolved");
        release.send(()).unwrap();
        let start = Instant::now();
        while !settled.load(Ordering::SeqCst) && start.elapsed() < Duration::from_secs(2) {
            std::thread::sleep(Duration::from_millis(1));
        }
        assert_eq!(program.function_call_traces, traces);
        assert!(settled.load(Ordering::SeqCst));
        assert!(socket.closed.load(Ordering::SeqCst));
        assert_eq!(socket.sent.lock().unwrap().len(), 1);
        Ok(())
    ...
    impl AxMCPTransport for ConcurrentMCPTransport {
        ...
        fn send(&mut self, message: Value) -> AxResult<Value> {
            let result = match message["method"].as_str().unwrap() {
                ...
                "initialize" => {
                    json!({"protocolVersion":"2025-11-25","serverInfo":{"name":"orders","version":"1"},"capabilities":{"tools":{}}})
                }
                "tools/list" => {
                    json!({"tools":[{"name":"lookup","inputSchema":{"type":"object","properties":{"index":{"type":"integer"}}}}]})
                }
                "tools/call" => {
                    assert_eq!(message["params"]["name"], "lookup");
                    self.0.lock().unwrap().push(message.clone());
                    json!({"resultType":"complete","_meta":{"io.modelcontextprotocol/serverInfo":{"name":"orders","version":message["id"]}},"structuredContent":message["params"]["arguments"]})
                }
    ...
    fn concurrent_native_mcp_request_ids() -> AxResult<()> {
        ...
        let mut client = AxMCPClient::new(
            Box::new(ConcurrentMCPTransport(requests.clone())),
            json!({"era":"modern","namespace":"orders"}),
        );
        client.init()?;
        let native = client.native_tools().remove(0);
        ...
        for index in 0..32 {
            ...
            workers.push(std::thread::spawn(move || {
                barrier.wait();
                native.call(json!({"index":index}))
            }));
        }
        for (index, worker) in workers.into_iter().enumerate() {
            let result = worker.join().expect("native MCP worker panicked")?;
            assert_eq!(result["structuredContent"]["index"], index);
        }
    ...
    fn owned_balancer_shares_failure_accounting() -> AxResult<()> {
        ...
        impl AxTransport for FailingTransport {
            fn owned_worker_factory(&self) -> Option<AxOwnedTransportFactory> {
                ...
                Some(Box::new(move || Box::new(FailingTransport(calls))))
            }
            fn send(&mut self, _: Value) -> AxResult<Value> {
                self.0.fetch_add(1, Ordering::SeqCst);
                let mut error = AxError::new("ai", "fixture rate limit");
                error.error_type = Some("AxAIServiceStatusError".into());
        ...
        let calls = Arc::new(AtomicUsize::new(0));
        let client = ai("openai", json!({"api_key":"test","model":"gpt-5.6"}))?
            .with_transport(FailingTransport(calls.clone()));
        let mut owner = AxBalancer::from_clients(
            vec![Box::new(client)],
            AxBalancerOptions {
                max_retries: 1,
        ...
        let mut worker = owner.owned_worker_factory().expect("built-in owned client")();
        let request = json!({"chat_prompt":[{"role":"user","content":"Hello"}],"model_config":{"stream":false}});
        assert!(worker.chat(request.clone()).is_err());
        let first = calls.load(Ordering::SeqCst);
        assert!(first > 0);
        assert!(owner.chat(request).is_err());
        assert_eq!(
    ...
    impl AxTransport for AgentSessionTransport {
        fn send(&mut self, request: Value) -> AxResult<Value> {
            ...
            for tool in body["tools"].as_array().into_iter().flatten() {
                assert_ne!(tool["async"], true, "actor authority leaked");
            }
            if n == 1 {
                return Ok(completed(
                    "distiller",
        ...
        fn stream(&mut self, request: Value) -> AxResult<AxTransportStream> {
            ...
            if matches!(n, 1 | 2 | 5 | 6) {
                for t in body["tools"].as_array().into_iter().flatten() {
                    assert_ne!(t["async"], true, "Actor authority leaked");
                }
                let stage = if n < 3 { "distiller" } else { "responder" };
                let suffix = if n == 1 || n == 5 { "-start" } else { "-final" };
                if n == 2 || n == 6 {
                    assert_eq!(body["previous_response_id"], format!("{stage}-start"));
                    let input = body["input"].to_string();
                    assert!(input.contains("ROOT-GUIDANCE"));
                    assert_eq!(input.contains("RESPONDER-ONLY"), n == 6);
                }
                if n == 5 {
                    assert!(
                        body.to_string().contains("REF-42"),
                        "Responder started before incorporation"
                    );
                }
            ...
            if n == 3 {
                ...
                std::thread::spawn(move || {
                    ...
                    release.send(()).unwrap();
                    sender.send(sse(completed("executor1","{\"completion\":{\"type\":\"final\",\"args\":[\"Report reference\",{\"answer\":\"provisional\"}]}}"))).unwrap();
                });
                return Ok(AxTransportStream::Reader {
                    status: 200,
                    body: Box::new(ChannelReader {
            ...
            assert_eq!(n, 4);
            assert_eq!(body["previous_response_id"], "executor1");
            assert_eq!(
                body["input"].as_array().unwrap().last().unwrap(),
                &json!({"type":"function_call_output","call_id":"agent-call","output":"REF-42"})
            );
            let input = body["input"].to_string();
            assert!(
                input.contains("ROOT-GUIDANCE")
                    && !input.contains("RESPONDER-ONLY")
                    && input.contains("configuration_update")
                    && input.contains("medium")
            );
            Ok(AxTransportStream::Buffered(
                json!({"status":200,"body":String::from_utf8(sse(completed("executor2","{\"completion\":{\"type\":\"final\",\"args\":[\"Report reference\",{\"answer\":\"REF-42\"}]}}"))).unwrap()}),
            ))
        }
    }
    ...
    impl AxCodeRuntime for ChildControlRuntime {
        ...
        fn create_session(&mut self, _: Value, _: Value) -> AxResult<Box<dyn AxCodeSession>> {
            Ok(Box::new(ChildControlSession {
        ...
        fn execute(&mut self, code: &str, _: Value) -> AxResult<RuntimeEnvelope> {
            if code == "delegate" {
                ...
                return Ok(RuntimeEnvelope {
                    payload: json!({"callable":{"qualified_name":"team.researcher","args":{"question":"Find reference"},"call_id":"child-call"}}),
                });
            }
            Ok(RuntimeEnvelope {
                payload: json!({"type":"final","args":["Find reference",{}]}),
            })
        }
        fn snapshot_globals(&mut self, _: Value) -> AxResult<Value> {
            Ok(json!({"globals":{}}))
        }
    ...
    impl AxTransport for ChildControlTransport {
        fn send(&mut self, _: Value) -> AxResult<Value> {
            Err(AxError::runtime("Expected session streaming"))
        }
        fn stream(&mut self, request: Value) -> AxResult<AxTransportStream> {
            let stages = [
                "root/distiller",
                "root/executor",
                "root/team.researcher/distiller",
                "root/team.researcher/executor",
                "root/team.researcher/responder",
                "root/executor",
                "root/responder",
            ];
            let body = request["json"].clone();
            let mut requests = self.requests.lock().unwrap();
            let number = requests.len();
            let stage = stages[number / 2];
            requests.push(body.clone());
            if number % 2 == 1 {
                assert_eq!(body["previous_response_id"], format!("child-r{number}"));
                let input = body["input"].to_string();
                assert!(input.contains("ROOT-UPDATE"));
                assert_eq!(
                    input.contains("CHILD-ONLY"),
                    stage.starts_with("root/team.researcher")
                );
                let updates: Vec<&Value> = body["input"]
                    .as_array()
                    .unwrap()
                    .iter()
                ...
                assert_eq!(
                    !updates.is_empty(),
                    stage == "root/team.researcher/executor"
                );
                if !updates.is_empty() {
                ...
                assert_eq!(
                    body["reasoning"],
                    requests[number - 1]["reasoning"],
                    "cache prefix changed"
                );
            } else {
                assert!(
                    body["previous_response_id"].is_null(),
                    "child inherited conversation at request {number}: {body}"
                );
            }
            if number == 10 {
                assert!(
                    body.to_string().contains("REF-42"),
                    "parent continued without child result"
                );
            }
            let output = if stage.starts_with("root/team.researcher") {
                if stage.ends_with("/responder") {
                    json!({"answer":"REF-42"})
                } else {
                    json!({"completion":{"type":"final","args":["Find reference",{}]}})
                }
            } else if stage == "root/responder" {
                json!({"answer":"REF-42"})
            } else {
    ...
    fn owned_child_controls_and_cancellation() -> AxResult<()> {
        for cancel in [false, true] {
            ...
            let result = parent.forward_with_options(
                &mut client,
                json!({"question":"Find reference"}),
                AxForwardOptions::default().with_control(control),
            );
            if cancel {
                let error = result.expect_err("cancelled child returned success");
                assert!(
                    error.to_string().to_lowercase().contains("abort"),
                    "{error}"
                );
                assert!((6..=7).contains(&requests.lock().unwrap().len()));
                assert_eq!(closed.load(Ordering::SeqCst), 1);
            } else {
                assert_eq!(result?, json!({"answer":"REF-42"}));
                assert_eq!(requests.lock().unwrap().len(), 14);
                assert_eq!(
                    observed
                        .lock()
                        .unwrap()
                ...
                let usage = parent.get_usage();
                let child = &usage["children"]["team.researcher"];
            ...
            let calls: Vec<Value> = parent
                .get_action_log()
                .into_iter()
                .filter(|item| item["call_id"] == "child-call")
                .collect();
            ...
            let error = parent
                ...
                .expect_err("parent retained active client");
            assert!(
                error.to_string().contains("active parent forward"),
                "{error}"
            );
        }
        Ok(())
    ...
    fn native_agent_tools_and_action_log() -> AxResult<()> {
        let control = run_control();
        control.steer("ROOT-GUIDANCE")?;
        control.steer_at("RESPONDER-ONLY", "root/responder")?;
        control.set_thinking_token_budget_at("medium", "root/executor")?;
        let requests = Arc::new(AtomicUsize::new(0));
        let calls = Arc::new(AtomicUsize::new(0));
        let called = calls.clone();
        let (started_tx, started_rx) = mpsc::channel();
        ...
        let lookup = tool("lookup")
            ...
            .handler(move |args| {
                ...
                release
                    ...
                    .unwrap()
                    .recv_timeout(Duration::from_secs(2))
                    .expect("model should overlap agent tool");
                Ok(args["query"].clone())
            });
        let mut client = ai("openai", json!({"api_key":"test","model":"gpt-6-astra"}))?
            .with_transport(AgentSessionTransport {
                requests: requests.clone(),
                started: Some(started_rx),
                release: release_tx,
            });
        ...
        let activity: Vec<Value> = program
            .get_action_log()
            .into_iter()
    ...
    fn cancellation_closes_stalled_native_http_body() -> AxResult<()> {
        ...
        let server = std::thread::spawn(move || {
            ...
            let body = sse(
                json!({"type":"response.output_item.done","item":{"type":"function_call","id":"item","call_id":"http-pending","name":"lookup","arguments":"{}"}}),
            );
            write!(socket, "{:x}\r\n", body.len()).unwrap();
        ...
        let mut client = ai(
            "openai",
            json!({"api_key":"test","model":"gpt-6-astra","api_url":endpoint}),
        )?;
        let mut program = ax("question -> answer")?.with_tool(lookup);
        let error = program
            .forward_with_options(
                &mut client,
                json!({"question":"Lookup"}),
                AxForwardOptions::from(json!({})).with_control(control),
            )
            .unwrap_err();
        ...
        assert!(closed_rx
            .recv_timeout(Duration::from_secs(3))
            .expect("HTTP worker retained its connection"));
        server.join().unwrap();
        Ok(())
    }

    struct BalancedChatOnly {
        calls: Arc<AtomicUsize>,
        tools: Arc<AtomicUsize>,
        unused: bool,
    }
    impl AxAIClient for BalancedChatOnly {
        fn get_features(&self, _: Option<&str>) -> Value {
            json!({"functions":true,"streaming":false,"asyncTools":self.unused})
        }
        fn chat(&mut self, request: Value) -> AxResult<Value> {
            assert!(!self.unused, "Pinned run changed providers");
    ...
    fn mixed_balancer_pins_chat_only_fallback() -> AxResult<()> {
        ...
        let mut client = AxBalancer::from_clients(
            vec![
                Box::new(BalancedChatOnly {
                    calls: calls.clone(),
                    tools: tools.clone(),
                    unused: false,
                }),
        ...
        let mut program = ax("question -> answer")?.with_tool(
            tool("lookup")
                .description("Lookup")
                .execution("background")
                .handler(move |_| {
        ...
        assert_eq!(
            program.forward(&mut client, json!({"question":"Lookup"}))?,
        ...
        tools.store(0, Ordering::SeqCst);
        let node = ax("question -> answer")?.with_tool(program.tools[0].clone());
        let mut workflow = flow("balanced-fallback")
            .execute("lookup", node)
            .returns(json!({"result":"lookupResult"}));
        assert_eq!(
            workflow.forward(&mut client, json!({"question":"Lookup"}))?,
            json!({"result":{"answer":"FALLBACK"}})
        );
        assert_eq!(calls.load(Ordering::SeqCst), 2);
        assert_eq!(unused.load(Ordering::SeqCst), 0);
        assert_eq!(tools.load(Ordering::SeqCst), 1);
