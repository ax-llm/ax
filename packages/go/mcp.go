...
func NewAxMCPClient(transport AxMCPTransport, options map[string]Value) *AxMCPClient {
	...
	c := &AxMCPClient{transport: transport, options: options, nextID: 1, notificationListeners:map[int]func(map[string]Value){}, lifecycleListeners:map[int]func(string){}, nextListenerID:1}
...
func (c *AxMCPClient) Close() error { c.initialized = false; return c.transport.Close() }
...
func (c *AxMCPClient) SubscribeResource(uri string)(map[string]Value,error){return c.request("resources/subscribe",map[string]Value{"uri":uri})}
func (c *AxMCPClient) UnsubscribeResource(uri string)(map[string]Value,error){return c.request("resources/unsubscribe",map[string]Value{"uri":uri})}
...
func (c *AxMCPClient) EmitLifecycle(state string){listeners:=make([]func(string),0,len(c.lifecycleListeners));for _,listener:=range c.lifecycleListeners{listeners=append(listeners,listener)};for _,listener:=range listeners{listener(state)}}
...
func (r *AxEventRuntime) RunDue()int{processed:=0;for{var due *axEventDelivery;for _,value:=range r.Store.Deliveries{if value.Status=="queued"&&value.AvailableAt<=r.Clock.Now()&&(due==nil||value.AvailableAt<due.AvailableAt||value.AvailableAt==due.AvailableAt&&value.Sequence<due.Sequence){due=value}};if due==nil{return processed};due.Status="running";r.Store.release(due);r.dispatch(due.Event,due.Command,due.IdentityScope,due.Trust);processed++}}
...
type AxMCPEventSource struct { Client *AxMCPClient; Namespace,IdentityScope,Trust string; Subscriptions []string; publish func(AxEventEnvelope,string,string)error; remove,removeLifecycle func(); nextID int }
func NewAxMCPEventSource(client *AxMCPClient,namespace,identityScope,trust string,subscriptions []string)*AxMCPEventSource{if namespace==""{namespace=client.Namespace()};if identityScope==""{identityScope="anonymous"};if trust==""{trust="untrusted"};return &AxMCPEventSource{Client:client,Namespace:namespace,IdentityScope:identityScope,Trust:trust,Subscriptions:append([]string(nil),subscriptions...),nextID:1}}
...
func (s *AxMCPEventSource) StartScoped(publish func(AxEventEnvelope,string,string)error)error{if err:=s.Client.Init();err!=nil{return err};s.publish=publish;s.remove=s.Client.AddNotificationListener(s.onNotification);s.removeLifecycle=s.Client.AddLifecycleListener(func(state string){if state=="reconnected"{_ = s.Reconnect()}});for _,uri:=range s.Subscriptions{if _,err:=s.Client.SubscribeResource(uri);err!=nil{return err}};return nil}
func (s *AxMCPEventSource) onNotification(message map[string]Value){method:=display(coreGet(message,"method",""));if method==""||s.publish==nil{return};normalized,err:=NormalizeMCPEvent(s.Namespace,method,coreGet(message,"params",Object()));if err!=nil{return};correlations:=[]map[string]string{};if raw:=asMap(coreGet(normalized,"correlation",Object()));len(raw)>0{correlations=append(correlations,map[string]string{"kind":display(raw["kind"]),"value":display(raw["value"])})};data:=coreGet(normalized,"data",Object());subject:=display(coreGet(data,"uri",coreGet(coreGet(data,"task",Object()),"taskId","")));event:=AxEventEnvelope{SpecVersion:"1.0",ID:fmt.Sprintf("mcp:%s:%d",s.Namespace,s.nextID),Source:display(normalized["source"]),Type:display(normalized["type"]),Subject:subject,Data:data,Correlation:correlations};s.nextID++;_ = s.publish(event,s.IdentityScope,s.Trust)}
func (s *AxMCPEventSource) Reconnect()error{for _,uri:=range s.Subscriptions{if _,err:=s.Client.SubscribeResource(uri);err!=nil{return err}};return nil}
func (s *AxMCPEventSource) Close()error{for _,uri:=range s.Subscriptions{_,_ = s.Client.UnsubscribeResource(uri)};if s.remove!=nil{s.remove()};if s.removeLifecycle!=nil{s.removeLifecycle()};s.remove=nil;s.removeLifecycle=nil;s.publish=nil;return nil}
