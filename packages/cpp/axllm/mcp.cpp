...
Value AxMCPClient::call_tool(const std::string& name, Value arguments) {auto params=object({{"name",name},{"arguments",arguments}});Value result;try{result=request_with_headers("tools/call",params,tool_headers(name,arguments),true);}catch(const AxError& error){if(era_!="modern"||error.code!="-32020")throw;tools_.clear();for(auto tool:collect_catalog("tools/list","tools")){try{Core::mcp_param_header_bindings(Core::get(tool,"inputSchema",Value::object()));tools_.push_back(tool);}catch(const std::exception&){}}result=request_with_headers("tools/call",params,tool_headers(name,arguments),true);}if(display(Core::get(result,"resultType",""))!="task")return result;if(!has_tasks_capability())throw AxError("mcp","MCP protocol violation: server returned a task without negotiating io.modelcontextprotocol/tasks");if(!Core::truthy(Core::mcp_validate_modern_task(result)))throw AxError("mcp","MCP protocol violation: invalid CreateTaskResult");return await_modern_task(display(Core::get(result,"taskId","")));}
