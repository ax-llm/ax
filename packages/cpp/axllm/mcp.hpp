...
class AxManualEventClock final:public AxEventClock {public:explicit AxManualEventClock(long value=0):current_(value){}long now()const override{std::lock_guard<std::mutex> lock(mutex_);return current_;}void advance(long milliseconds){{std::lock_guard<std::mutex> lock(mutex_);current_+=milliseconds;}condition_.notify_all();}bool sleep(long milliseconds,const AxEventCancellationToken* cancellation=nullptr)override{auto subscription=cancellation?cancellation->subscribe([this]{std::lock_guard<std::mutex> lock(mutex_);condition_.notify_all();}):AxCancellationToken::Subscription{};std::unique_lock<std::mutex> lock(mutex_);auto target=current_+std::max(0L,milliseconds);condition_.wait(lock,[&]{return current_>=target||(cancellation&&cancellation->is_cancelled());});return !cancellation||!cancellation->is_cancelled();}private:mutable std::mutex mutex_;std::condition_variable condition_;long current_;};
