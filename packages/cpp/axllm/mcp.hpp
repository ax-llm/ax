...
using AxEventCancellationToken = AxCancellationToken;
...
class AxSystemEventClock final:public AxEventClock {public:long now()const override{return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();}bool sleep(long milliseconds,const AxEventCancellationToken* cancellation=nullptr)override{auto duration=std::chrono::milliseconds(std::max(0L,milliseconds));if(!cancellation){std::this_thread::sleep_for(duration);return true;}if(cancellation->is_cancelled())return false;return !cancellation->wait_for(duration);}};
// sleep() fixes its deadline before subscribing (like every other port): once a
// sleeper shows up on its token, a later advance() is guaranteed to release it.
class AxManualEventClock final:public AxEventClock {public:explicit AxManualEventClock(long value=0):current_(value){}long now()const override{std::lock_guard<std::mutex> lock(mutex_);return current_;}void advance(long milliseconds){{std::lock_guard<std::mutex> lock(mutex_);current_+=milliseconds;}condition_.notify_all();}bool sleep(long milliseconds,const AxEventCancellationToken* cancellation=nullptr)override{auto target=now()+std::max(0L,milliseconds);auto subscription=cancellation?cancellation->subscribe([this]{std::lock_guard<std::mutex> lock(mutex_);condition_.notify_all();}):AxCancellationToken::Subscription{};std::unique_lock<std::mutex> lock(mutex_);condition_.wait(lock,[&]{return current_>=target||(cancellation&&cancellation->is_cancelled());});return !cancellation||!cancellation->is_cancelled();}private:mutable std::mutex mutex_;std::condition_variable condition_;long current_;};
...
class AxMCPEventSource final:public AxEventSource,public AxScopedEventSource {
 public:
  ...
  ~AxMCPEventSource()override{close();}
  ...
  void start_scoped(std::function<void(AxEventEnvelope,std::string,std::string)> publish)override{closing_=false;client_->init();publish_=std::move(publish);listener_id_=client_->add_notification_listener([this](Value message){on_notification(std::move(message));});lifecycle_listener_id_=client_->add_lifecycle_listener([this](const std::string& state){if(state=="reconnected")queue_reconcile();});if(client_->get_era()=="modern"){reconcile();client_->start_listening();}else reconcile();}
  void reconnect(){client_->restore_resource_subscriptions();reconcile();}
  void close()override{closing_=true;if(listener_id_>0)client_->remove_notification_listener(listener_id_);if(lifecycle_listener_id_>0)client_->remove_lifecycle_listener(lifecycle_listener_id_);listener_id_=0;lifecycle_listener_id_=0;std::thread worker;{std::lock_guard<std::mutex> lock(reconcile_mutex_);if(reconcile_thread_.joinable())worker=std::move(reconcile_thread_);}if(worker.joinable())worker.join();for(const auto& uri:subscriptions_){try{client_->release_resource_subscription(uri,owner_);}catch(...){}}subscriptions_.clear();publish_={};}
 private:
