...
def mcp_validate_modern_task(task: Any) -> bool:
    _core_coverage_mark("mcp_validate_modern_task")
    is_object = _core_type_is(task, "object")
    if is_object:
        task_id = _core_get(task, "taskId", None)
        status = _core_get(task, "status", None)
        created_at = _core_get(task, "createdAt", None)
        updated_at = _core_get(task, "lastUpdatedAt", None)
        task_id_string = _core_type_is(task_id, "string")
        created_string = _core_type_is(created_at, "string")
        updated_string = _core_type_is(updated_at, "string")
        working = _core_eq(status, "working")
        input_required = _core_eq(status, "input_required")
        completed = _core_eq(status, "completed")
        failed = _core_eq(status, "failed")
        cancelled = _core_eq(status, "cancelled")
        status_a = _core_or(working, input_required)
        status_b = _core_or(completed, failed)
        status_c = _core_or(status_a, status_b)
        status_valid = _core_or(status_c, cancelled)
        has_ttl = _core_map_contains(task, "ttlMs")
        ttl = _core_get(task, "ttlMs", None)
        ttl_null = _core_eq(ttl, None)
        ttl_number = _core_type_is(ttl, "number")
        ttl_type = _core_or(ttl_null, ttl_number)
        ttl_valid = _core_and(has_ttl, ttl_type)
        has_poll = _core_map_contains(task, "pollIntervalMs")
        poll = _core_get(task, "pollIntervalMs", None)
        poll_number = _core_type_is(poll, "number")
        no_poll = _core_not(has_poll)
        poll_valid = _core_or(no_poll, poll_number)
        identity_valid = _core_and(task_id_string, status_valid)
        dates_valid = _core_and(created_string, updated_string)
        base_valid = _core_and(identity_valid, dates_valid)
        cache_valid = _core_and(ttl_valid, poll_valid)
        valid = _core_and(base_valid, cache_valid)
        return valid
    else:
        pass
    return False
...
def mcp_task_terminal_outcome(task: Any) -> Any:
    ...
    out = {}
    task_id = _core_get(task, "taskId", "")
    status = _core_get(task, "status", "")
    completed = _core_eq(status, "completed")
    if completed:
        has_result = _core_map_contains(task, "result")
        if has_result:
            out["kind"] = "result"
            result = _core_get(task, "result", None)
            out["result"] = result
        else:
            out["kind"] = "violation"
            message = _core_string_format("MCP protocol violation: completed task {} omitted result", task_id)
            out["message"] = message
        return out
    else:
        pass
    failed = _core_eq(status, "failed")
    if failed:
        error = _core_get(task, "error", None)
        error_object = _core_type_is(error, "object")
        if error_object:
            code = _core_get(error, "code", None)
            error_message = _core_get(error, "message", None)
            code_number = _core_type_is(code, "number")
            message_string = _core_type_is(error_message, "string")
            typed_error = _core_and(code_number, message_string)
            if typed_error:
                out["kind"] = "protocol_error"
                out["code"] = code
                out["message"] = error_message
                has_data = _core_map_contains(error, "data")
                if has_data:
                    data = _core_get(error, "data", None)
                    out["data"] = data
                else:
                    pass
                return out
            ...
            pass
        out["kind"] = "failure"
        message = _core_string_format("MCP task {} failed", task_id)
        out["message"] = message
        return out
    else:
        pass
...
def mcp_listen_interests(subscribed_uris: list[Any], filters: Any) -> Any:
    ...
    for uri in subscribed_uris:
        ...
        if uri_string:
            empty = _core_eq(uri, "")
            duplicate = _core_contains(subscriptions, uri)
            skip = _core_or(empty, duplicate)
            if skip:
                pass
            else:
            ...
            pass
    count = _core_len(subscriptions)
    has_subscriptions = _core_gt(count, 0)
    if has_subscriptions:
        out["resourceSubscriptions"] = subscriptions
    else:
        _core_map_delete(out, "resourceSubscriptions")
    return out


def mcp_notification_subscription_filter(message: Any, active_subscription_id: str) -> Any:
    _core_coverage_mark("mcp_notification_subscription_filter")
    out = {}
    out["deliver"] = True
    out["acknowledged"] = False
    none = _core_none()
    out["subscriptionId"] = none
    message_text = _core_json_stringify(message)
    clean_message = _core_json_parse(message_text)
    message_object = _core_type_is(clean_message, "object")
    if message_object:
        params = _core_get(clean_message, "params", None)
        params_object = _core_type_is(params, "object")
        if params_object:
            meta = _core_get(params, "_meta", None)
            meta_object = _core_type_is(meta, "object")
            if meta_object:
                subscription_id = _core_get(meta, "io.modelcontextprotocol/subscriptionId", None)
                subscription_string = _core_type_is(subscription_id, "string")
                if subscription_string:
                    out["subscriptionId"] = subscription_id
                    active_missing = _core_is_none(active_subscription_id)
                    if active_missing:
                        pass
                    else:
                        same_subscription = _core_eq(subscription_id, active_subscription_id)
                    ...
                    pass
                _core_map_delete(meta, "io.modelcontextprotocol/subscriptionId")
                meta_keys = _core_map_keys(meta)
                meta_count = _core_len(meta_keys)
                meta_empty = _core_eq(meta_count, 0)
                if meta_empty:
                    _core_map_delete(params, "_meta")
                else:
                    pass
            else:
                pass
            ...
            pass
        method = _core_get(clean_message, "method", "")
        is_ack = _core_eq(method, "notifications/subscriptions/acknowledged")
        delivered = _core_get(out, "deliver", False)
        ack_deliver = _core_and(is_ack, delivered)
        if ack_deliver:
            params = _core_get(clean_message, "params", None)
            params_object = _core_type_is(params, "object")
            if params_object:
                notifications = _core_get(params, "notifications", None)
                notifications_object = _core_type_is(notifications, "object")
                if notifications_object:
                    received = _core_get(out, "subscriptionId", None)
                    received_string = _core_type_is(received, "string")
                    active_missing = _core_is_none(active_subscription_id)
                    matches_active = False
                    if active_missing:
                        matches_active = received_string
                    else:
                        matches_active = _core_eq(received, active_subscription_id)
                    if matches_active:
...
def mcp_oauth_validate_issuer(response: Any, expected_issuer: str, require_iss: bool) -> Any:
    ...
    expected_state = _core_get(response, "expectedState", None)
    state_matches = _core_eq(state, expected_state)
    if state_matches:
        pass
    else:
        out = {}
        out["ok"] = False
        out["message"] = "OAuth state mismatch"
        return out
    returned_issuer = _core_get(response, "iss", None)
    issuer_missing = _core_is_none(returned_issuer)
    if issuer_missing:
        if require_iss:
            out = {}
            out["ok"] = False
            out["message"] = "OAuth authorization response issuer is required"
            return out
        else:
            pass
    else:
        issuer_matches = _core_eq(returned_issuer, expected_issuer)
        if issuer_matches:
            pass
        else:
            message = _core_string_format("OAuth issuer mismatch: expected {}, received {}", expected_issuer, returned_issuer)
            out = {}
            out["ok"] = False
            out["message"] = message
            return out
