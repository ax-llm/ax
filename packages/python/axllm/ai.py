...
def _openai_build_chat_request_impl(request: AxChatRequest, options: Any, prompt_caching: bool, reasoning_content_mode: str, reasoning_details_mode: str) -> Any:
    ...
    message_index = 0
    marker_count = 0
    extended_media = _core_string_starts_with(model, "muse-")
    for message in chat_prompt:
        provider_message = _openai_message_impl(message, reasoning_content_mode, reasoning_details_mode, extended_media)
        if cache_enabled:
...
def _openai_message_impl(message: Any, reasoning_content_mode: str, reasoning_details_mode: str, extended_media: bool) -> Any:
    ...
    if is_user:
        ...
        if content_is_list:
            parts = []
            for part in content:
                provider_part = _openai_content_part_impl(part, extended_media)
...
def _openai_content_part_impl(part: Any, extended_media: bool) -> Any:
    ...
    is_supported_file = _core_and(is_file, extended_media)
