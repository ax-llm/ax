...
func (c *openAIBackedAgentClient) Chat(ctx context.Context, request map[string]ax.Value, options map[string]ax.Value) (ax.Value, error) {
	c.calls++
	if c.rawModelAnswer == "" {
		response, err := c.inner.Chat(ctx, map[string]ax.Value{"chat_prompt": ax.Array(ax.Object("role", "user", "content", "Use local context to choose between generation, agents, and flows."))}, nil)
...
func main() {
	...
	assistant := ax.NewAgent("question:string -> answer:string, usedContext:boolean", map[string]ax.Value{"contextFields": ax.Array()})
	output, err := assistant.Forward(ctx, stageClient, map[string]ax.Value{"question": "Use local context to choose between generation, agents, and flows."}, nil)
	if err != nil { panic(err) }
