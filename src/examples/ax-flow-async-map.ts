...
const multiApiFlow = new AxFlow<
  { productId: string },
  { userReviews: string; productDetails: string; pricing: string }
>().map([
  async (state) => {
  ...
  async (state) => {
    ...
    return { ...state, productDetails: `Details for product ${state.productId}` };
  },
  async (state) => {
...
>()
  .node('validator', 'inputData:string -> isValid:boolean, validationMessage:string')
  ...
  .map(async (state) => {
    ...
    return {
      ...
      validationStatus: aiValid && externalValid ? 'fully-validated' : 'validation-failed'
    };
  });
...
>()
  ...
  .map(async (state) => {
    ...
    const completed = [
      ...(state.highPriorityCompleted || []),
      ...(state.mediumPriorityCompleted || [])
    ];
...
console.log(`⏱️  Total time: ${Date.now() - startTime2}ms (should be ~120ms, not 260ms if run in parallel)`);
