...
fn main() -> AxResult<()> {
    let repo_root = env::var("AXIR_REPO_ROOT")
        .map_err(|_| axllm::AxError::runtime("AXIR_REPO_ROOT is required"))?;
    let server = env::var("AXIR_AXJS_RUNTIME_SERVER")
        .map_err(|_| axllm::AxError::runtime("AXIR_AXJS_RUNTIME_SERVER is required"))?;
    let mut runtime =
        ProcessCodeRuntime::new(["node".to_string(), "--import=tsx".to_string(), server]);
