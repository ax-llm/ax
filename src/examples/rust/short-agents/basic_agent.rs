...
impl axllm::AxAIClient for OpenAIBackedAgentClient {
    fn chat(&mut self, _request: serde_json::Value) -> AxResult<serde_json::Value> {
        ...
        if self.raw_model_answer.is_none() {
            let response = self.inner.chat(json!({"chat_prompt": [{"role": "user", "content": "What does Ax make easier when building production LLM features?"}]}))?;
...
fn main() -> AxResult<()> {
    ...
    let output = assistant.forward(&mut stage_client, json!({"question": "What does Ax make easier when building production LLM features?"}))?;
