...
export function getAutocompleteItems(content: string, cursorPosition: number): AutocompleteItem[] {
...
function analyzeContext(beforeCursor: string): { type: string; partial?: string } {
  ...
  const lastSpaceAfterColon = beforeCursor.substring(lastColonIndex).indexOf(' ');
  
  if (lastColonIndex !== -1 && (lastSpaceAfterColon === -1 || lastSpaceAfterColon > lastColonIndex)) {
  ...
  if (beforeCursor.includes('class(') && !beforeCursor.substring(beforeCursor.lastIndexOf('class(')).includes(')')) {
    return { type: 'classOptions' };
  }
  
  // Check if we're in a code definition
  if (beforeCursor.includes('code(') && !beforeCursor.substring(beforeCursor.lastIndexOf('code(')).includes(')')) {
...
function getFieldTypeCompletions(): AutocompleteItem[] {
  return [
    ...
    },
    {
      ...
      documentation: 'An audio field for audio file uploads (input fields only)',
      insertText: 'audio',
      kind: 'type'
    },
    {
    ...
    },
    {
      ...
      documentation: 'A classification field with predefined options (output fields only)',
  ...
  return [
    {
      ...
      insertText: 'inputText:string "Text to analyze" -> sentimentCategory:class("positive", "negative", "neutral"), confidenceScore:number "Confidence 0-1"',
      kind: 'template'
    },
    {
      ...
      insertText: 'problemDescription:string "Programming problem to solve" -> pythonSolution:code("python") "Python code solution", solutionExplanation:string "Explanation of approach"',
      kind: 'template'
    },
    {
      ...
      insertText: 'customerFeedback:string "Customer feedback text" -> extractedTopics:string[] "Topics mentioned", urgencyLevel:class("low", "medium", "high"), actionItems?:string[] "Required actions"',
      kind: 'template'
    },
    {
      ...
      insertText: 'documentText:string "Document to summarize" -> documentSummary:string "Concise summary", keyPoints:string[] "Main points", wordCount:number "Summary word count"',
      kind: 'template'
    },
    {
      ...
      insertText: 'contextText:string "Context information", userQuestion:string "Question to answer" -> answer:string "Answer based on context", confidence:number "Answer confidence 0-1"',
      kind: 'template'
    },
    {
      ...
      insertText: 'sourceText:string "Text to translate", targetLanguage:string "Target language" -> translatedText:string "Translated text", sourceLanguage:string "Detected source language"',
      kind: 'template'
    }
  ];
}
...
function getCodeLanguageCompletions(): AutocompleteItem[] {
  return [
    { label: 'python', detail: 'Python code', insertText: '"python"', kind: 'template' },
    { label: 'javascript', detail: 'JavaScript code', insertText: '"javascript"', kind: 'template' },
    { label: 'typescript', detail: 'TypeScript code', insertText: '"typescript"', kind: 'template' },
    { label: 'java', detail: 'Java code', insertText: '"java"', kind: 'template' },
    { label: 'cpp', detail: 'C++ code', insertText: '"cpp"', kind: 'template' },
    { label: 'rust', detail: 'Rust code', insertText: '"rust"', kind: 'template' },
