...
describe('TypeScript Parser Parity with JS Parser', () => {
  test('should handle all the same features as JS parser', () => {
    ...
    console.log(`✅ JS Parser successfully handled ${testCases.length} test cases`);
  });
  ...
  test('should demonstrate TypeScript type inference capabilities', () => {
    ...
    import('./sigtypes.js').then(({ ParseSignature }) => {
      ...
      type BasicTest = ParseSignature<'userQuestion:string -> responseText:string'>;
      ...
      type SimpleClassTest = ParseSignature<'userQuestion:string -> category:class "positive"'>;
