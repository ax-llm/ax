...
import type { AxGenDeltaOut, AxProgramUsage, DeltaOut } from './program.js'
...
export function mergeDeltas<OUT extends AxGenOut>(
    ...
    const target = base.find((b) => b.index === index)?.delta as Record<string, ValueTypeOfAxGenOut>
    ...
    for (const key of Object.keys(delta)) {
        ...
        if (baseValue === undefined) {
            throw new Error(`No base value found for key ${key} at index ${index}`)
        }
