...
describe('Fluent API Practical Type Verification', () => {
  it('should create correct field types for arrays and demonstrate compilation success', () => {
    ...
    const regularStringField = inputFields.find(
      (f) => f.name === 'regularString'
    );
    ...
    const optionalStringField = inputFields.find(
      (f) => f.name === 'optionalString'
    );
  ...
  it('should demonstrate the type safety works at compile time', () => {
    ...
    const inputFields = sig.getInputFields();
    expect(inputFields.find((f) => f.name === 'strings')?.type?.name).toBe(
      'string'
    );
    expect(inputFields.find((f) => f.name === 'strings')?.type?.isArray).toBe(
      true
    );
    expect(inputFields.find((f) => f.name === 'numbers')?.type?.name).toBe(
      'number'
    );
    expect(inputFields.find((f) => f.name === 'numbers')?.type?.isArray).toBe(
      true
    );
    expect(inputFields.find((f) => f.name === 'booleans')?.type?.name).toBe(
      'boolean'
    );
    expect(inputFields.find((f) => f.name === 'booleans')?.type?.isArray).toBe(
      true
    );
  });
});
