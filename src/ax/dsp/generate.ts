...
import { type AxChatResponseFunctionCall, type AxInputFunctionType, createFunctionConfig, parseFunctions } from './functions.js'
import {
    processResponse,
...
> extends AxProgramWithSignature<IN, OUT> {
    ...
    private async forwardSendRequest({
        ...
        const {
            ...
            rateLimiter,
            stream,
            functionCall: definedFunctionCall,
            thinkingTokenBudget,
            showThoughts,
        } = options ?? {}
    ...
    private async *forwardCore({
        ...
        const { functions, functionCall } = createFunctionConfig(functionList, definedFunctionCall, firstStep)
