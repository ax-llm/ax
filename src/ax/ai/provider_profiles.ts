...
// DeepSeek V4 supports a distinct medium reasoning effort. Keep this
// handwritten compatibility override until the provider profile is migrated
// into AxIR and regenerated by a maintainer-owned change.
const deepseekV4Profile = profiles.deepseek;
const deepseekV4Rule = deepseekV4Profile?.modelRules.find((rule) =>
  rule.match.exact?.includes('deepseek-v4-flash')
);
if (deepseekV4Rule?.request?.effortMap) {
  deepseekV4Rule.request.effortMap = {
    ...deepseekV4Rule.request.effortMap,
    medium: 'medium',
  };
}
