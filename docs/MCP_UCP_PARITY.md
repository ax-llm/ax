...
| MCP lifecycle | Server input requests and notifications | Implemented | TypeScript legacy roots/sampling/elicitation and modern MRTR input rounds; generated clients use the Core server-request planner and optional transport callback for legacy ping/roots/elicitation |
...
| Languages | Legacy inbound requests | Implemented | `axmcp/server-requests-legacy.json`; generated built-in and scripted transports answer ping, roots, and elicitation through the shared planner, returning `-32601` for missing/unsupported handlers and `-32603` for handler failures |
...
| Languages | Modern Tasks v2 | Implemented | `axmcp/tasks-v2-modern.json`, `axmcp/tasks-v2-input-required.json`, and `axmcp/tasks-v2-violations.json`; generated clients fulfill roots and elicitation through `tasks/update`, reject unsupported sampling, resume polling, and consume the flattened terminal result |
| Languages | Modern MRTR roots and elicitation tranche | Implemented | `axmcp/mrtr-roots.json`, `axmcp/mrtr-elicitation.json`, and `axmcp/mrtr-violations.json`; the five-language modern localhost smoke fulfills roots automatically, invokes a host elicitation callback, resumes the original tool call with the byte-exact request state, and rejects sampling because the ports do not advertise or implement it |
