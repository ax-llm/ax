...
- `axir-2026-07-18-port-axflow-mermaid-dialect-tomermaid-frommermaid` [axflow] Port AxFlow mermaid dialect (toMermaid + fromMermaid)
  ...
  - Impact: TS AxFlow steps now carry inspectable build-time meta (branch bodies, loop bodies, feedback targets) and AxFlow.toMermaid() renders flows as mermaid flowcharts with %%ax signature directives; flow.fromMermaid() compiles the dialect back into runnable flows. The primary TS surface is the shorthand: flow(mermaidText, bindings?) compiles (string arg dispatches to fromMermaid) and AxFlow.toString() returns the mermaid source (String(wf) round-trips via flow(String(wf))). Generated Python/Java/C++/Go/Rust flow surfaces cannot emit or load the dialect. Parity means tools/axir template + codegen builder + golden updates, including each language's idiomatic equivalent of the shorthand (constructor/str dunder etc.) alongside the explicit toMermaid/fromMermaid names.
  - Suggested AxIR work: Add or update the TS-derived conformance fixture.; Update AxIR/Core or descriptor data to match the portable TS behavior.; Run npm run axir:conformance:check and npm run test:axir.
