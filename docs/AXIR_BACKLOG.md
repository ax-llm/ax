...
- `axir-2026-09-24-prefer-the-axoutput-rung-beside-callable-gemini-tools` [axgen] Prefer the __axOutput rung beside callable Gemini tools
  ...
  - TS paths: `src/ax/dsp/generate.ts`, `src/ax/ai/google-gemini/api.ts`, `src/ax/ai/base.ts`, `src/ax/ai/balance.ts`, `src/ax/ai/mock/api.ts`, `src/ax/dsp/generate.fallback-function.test.ts`, `src/ax/ai/google-gemini/api.structuredOutput.test.ts`, `src/ax/ai/balance.test.ts`
  - Impact: TypeScript AxGen now picks the function rung (__axOutput) instead of the native JSON Schema rung in structuredOutputMode 'auto' when the provider reports responseFormatWithFunctions: false (the Gemini client does) and the model may call native user functions (functionCall unset or 'auto', not prompt-emulated). Gemini 2.x rejects responseMimeType application/json beside function_declarations with HTTP 400, and gemini-3.8-flash and gemini-3.1-flash-lite keep re-calling tools until maxSteps once function results are in a JSON-mode conversation; the function rung finished every live run on eight Gemini models. The generated ports still select the native rung for complex signatures with Gemini tools, so they 400 on Gemini 2.x and loop on 3.8-flash and 3.1-flash-lite.
  - Suggested AxIR work: Add responseFormatWithFunctions: false to the google-gemini profile capabilities in ir/axcore/data/provider-profiles.json.; Make @select_structured_output_rung return the function rung when native user functions stay callable and that capability is false (reuse the functions-aware selector from PR #684).; Add forward conformance fixtures: the Gemini auto rung is function beside callable tools and stays native without tools or with a forced functionCall.; Run npm run axir:conformance:check and npm run test:axir.
