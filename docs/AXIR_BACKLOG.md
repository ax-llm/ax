...
- `axflow-mermaid-ts-only` [axflow]
  - Reason: Mermaid flow authoring (flow(text) compile + toString render dialect) is a TypeScript-only convenience by design; AxIR ports intentionally do not implement the dialect
  - Paths: `src/ax/flow/mermaid.ts`
  - Scoped files: `src/ax/flow/flow.fromMermaid.test.ts`, `src/ax/flow/flow.toMermaid.test.ts`
  - Tags: `typescript-only`, `mermaid`
