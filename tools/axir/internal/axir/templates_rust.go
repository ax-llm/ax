...
impl AxGen {
    ...
    pub fn forward<C: AxAIClient>(&mut self, client: &mut C, input: Value) -> AxResult<Value> {
        ...
        for attempt in 0..3 {
            ...
            self.memory.push(json!({"role": "request", "request": request.clone()}));
            let response = client.chat(request.clone())?;
            self.memory.push(json!({"role": "assistant", "response": response.clone()}));
            ...
            if !calls.is_empty() {
                ...
                for call in calls {
                    ...
                    if self.stop_functions.iter().any(|candidate| candidate == name) {
...
fn run_prompt_fixture(fixture: &Value) -> AxResult<()> {
    ...
    let prompt_text = if let Some(expected) = fixture.get("expected_messages") {
        if !expected.is_array() {
            return Err(AxError::new("fixture", "prompt expected_messages must be an array"));
        }
...
fn render_fixture_template(template: &str, vars: &Value) -> AxResult<String> {
    ...
    while let Some(start) = out.find("{{") {
        ...
        let replacement = if tag.starts_with('!') {
            ...
            if !tag.chars().all(|ch| ch.is_ascii_alphanumeric() || ch == '_' || ch == '.') {
