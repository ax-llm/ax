...
func skillFlowPatterns(target string) []skillPattern {
	...
	case "python":
		return []skillPattern{
			...
			pattern("Fan-out and join", "Independent reads place research and audience analysis in one planner group; the generated runtime currently executes its nodes serially.", readmeLines("parallel_flow = (", "    flow({\"id\": \"docs.parallelFlow\"})", "    .execute(\"research\", research, {\"reads\": [\"topicText\"], \"writes\": [\"researchResult\", \"factList\"]})", "    .execute(\"audience\", audience, {\"reads\": [\"topicText\"], \"writes\": [\"audienceResult\", \"audienceAngle\"]})", "    .execute(\"join\", join, {\"reads\": [\"factList\", \"audienceAngle\"], \"writes\": [\"joinResult\", \"briefText\"]})", "    .returns({\"briefText\": \"briefText\"})", ")")),
	...
	case "rust":
		return []skillPattern{
			...
			pattern("Fan-out and join", "Independent reads place research and audience analysis in one planner group; the generated runtime currently executes its nodes serially.", readmeLines("let mut parallel_flow = axllm::flow(\"docs.parallelFlow\")", "    .execute_with_options(\"research\", research, &json!({\"reads\": [\"topicText\"], \"writes\": [\"researchResult\", \"factList\"]}))", "    .execute_with_options(\"audience\", audience, &json!({\"reads\": [\"topicText\"], \"writes\": [\"audienceResult\", \"audienceAngle\"]}))", "    .execute_with_options(\"join\", join, &json!({\"reads\": [\"factList\", \"audienceAngle\"], \"writes\": [\"joinResult\", \"briefText\"]}))", "    .returns(json!({\"briefText\": \"briefText\"}));")),
