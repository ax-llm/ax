...
fn main() -> AxResult<()> {
    ...
    let meta = ai("meta", json!({"api_key": "test-key", "model": "muse-voice-transcribe-1.0"}))?;
    ...
    if meta_final["remote_session_id"] != "meta-session" || meta_final["results"][0]["content"] != "Hello world!" || meta_final["results"][1]["content"] != "Second turn" {
