...
mod tests {
    ...
    fn discovered_mcp_native_agent_invocation()->AxResult<()> {
        ...
        let mut mcp=AxMCPClient::new(Box::new(MCPAgentTransport(gate.clone())),json!({"era":"modern","namespace":"orders"}));mcp.init()?;let mut native=mcp.native_tools().remove(0);assert_eq!(native.execution,"blocking");native.execution="background".into();
        ...
        assert!(program.get_action_log().iter().any(|entry|entry["qualified_name"]=="orders.lookup"&&entry["call_id"]=="mcp-call"&&entry["status"]=="ok"));assert_eq!(program.invoke_callable("orders.lookup",json!({"query":"REF-42"}),json!({}))?["status"],"error");assert_eq!(gate.calls.load(Ordering::SeqCst),1);Ok(())
    }
