...
fn main() -> AxResult<()> {
    ...
    let limiter: Arc<dyn AxRateLimiter> = Arc::new(move |next: &mut dyn FnMut() -> AxResult<serde_json::Value>, info: &axllm::AxRateLimitInfo| {
    ...
    let hooks = AxRuntimeHooks { rate_limiter: Some(limiter), tracer: None, meter: None };
    ax_with_runtime_hooks("input:string -> output:string", hooks.clone())?.set_tracer(None).set_meter(None);
    agent_with_runtime_hooks("input:string -> output:string", json!({}), hooks.clone())?.set_tracer(None).set_meter(None);
    flow_with_runtime_hooks("runtime-hooks", hooks).set_tracer(None).set_meter(None);
