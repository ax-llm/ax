...
class AxMCPClient {
 public:
  ...
  void emit_lifecycle(const std::string& state){auto listeners=lifecycle_listeners_;for(auto& item:listeners)item.second(state);}
...
class AxMCPEventSource final:public AxEventSource,public AxScopedEventSource {public:AxMCPEventSource(std::shared_ptr<AxMCPClient> client,std::string namespace_name="",std::string identity_scope="anonymous",std::string trust="untrusted",std::vector<std::string> subscriptions={}):client_(std::move(client)),namespace_(namespace_name.empty()?client_->namespace_name():std::move(namespace_name)),identity_scope_(std::move(identity_scope)),trust_(std::move(trust)),subscriptions_(std::move(subscriptions)){}void start(std::function<void(AxEventEnvelope)> publish)override{start_scoped([publish=std::move(publish)](AxEventEnvelope event,std::string,std::string){publish(std::move(event));});}void start_scoped(std::function<void(AxEventEnvelope,std::string,std::string)> publish)override{client_->init();publish_=std::move(publish);listener_id_=client_->add_notification_listener([this](Value message){on_notification(std::move(message));});lifecycle_listener_id_=client_->add_lifecycle_listener([this](const std::string& state){if(state=="reconnected")reconnect();});reconnect();}void reconnect(){for(const auto& uri:subscriptions_)client_->subscribe_resource(uri);}void close()override{for(const auto& uri:subscriptions_){try{client_->unsubscribe_resource(uri);}catch(...){}}if(listener_id_>0)client_->remove_notification_listener(listener_id_);if(lifecycle_listener_id_>0)client_->remove_lifecycle_listener(lifecycle_listener_id_);listener_id_=0;lifecycle_listener_id_=0;publish_={};}private:void on_notification(Value message){if(!publish_)return;auto method=display(Core::get(message,"method",""));if(method.empty())return;auto normalized=Core::event_normalize_mcp(namespace_,method,Core::get(message,"params",Value::object()));AxEventEnvelope event;event.id="mcp:"+namespace_+":"+std::to_string(next_id_++);event.source=display(Core::get(normalized,"source"));event.type=display(Core::get(normalized,"type"));event.data=Core::get(normalized,"data",Value::object());event.subject=display(Core::get(event.data,"uri",Core::get(Core::get(event.data,"task",Value::object()),"taskId","")));auto key=Core::get(normalized,"correlation",Value());if(!key.is_null())event.correlation.push_back({display(Core::get(key,"kind")),display(Core::get(key,"value"))});publish_(std::move(event),identity_scope_,trust_);}std::shared_ptr<AxMCPClient> client_;std::string namespace_,identity_scope_,trust_;std::vector<std::string> subscriptions_;std::function<void(AxEventEnvelope,std::string,std::string)> publish_;int listener_id_=0;int lifecycle_listener_id_=0;int next_id_=1;};
class AxEventRuntime {
 public:
  ...
  int run_due(){int processed=0;while(true){AxEventDelivery* due=nullptr;for(auto& item:store_.deliveries){auto& value=item.second;if(value.status=="queued"&&value.availableAt<=clock_->now()&&(!due||value.availableAt<due->availableAt||(value.availableAt==due->availableAt&&value.sequence<due->sequence)))due=&value;}if(!due)return processed;due->status="running";store_.release(*due);dispatch(due->event,due->command,due->identityScope,due->trust);++processed;}}
