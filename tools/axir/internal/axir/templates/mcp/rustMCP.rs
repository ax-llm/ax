...
impl AxMCPClient {
    ...
    pub fn close(&mut self)->AxResult<()>{self.initialized=false;self.subscription_owners.lock().unwrap().clear();self.catalog_cache.clear();self.resource_read_cache.clear();self.transport.lock().unwrap().close()}
