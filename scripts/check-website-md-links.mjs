...
const destination = await mkdtemp(path.join(tmpdir(), 'website-md-public-'));

try {
  run('npm', ['run', 'doc:build:markdown']);
  run('npm', ['run', 'website-md:prepare']);
  run('hugo', [
    '--source',
    siteRoot,
    '--environment',
  ...
  if (failures.length > 0) {
    console.error('website-md local link check failed:');
  ...
  if (qualityFailures.length > 0) {
    console.error('website-md quality check failed:');
  ...
  console.log(
    `website-md link and quality checks passed (${htmlFiles.length} HTML files).`
  );
} finally {
...
function run(command, args) {
  ...
  if (result.error) {
    if (result.error.code === 'ENOENT' && command === 'hugo') {
      console.error(
        'Hugo is required for website-md checks. Install Hugo v0.162.0 or run in CI.'
      );
    }
    throw result.error;
  }
