...
console.log(
  `website-md generated content: ${path.relative(repoRoot, generatedContentRoot)}`
);

async function writeGeneratedDocPage(language, page) {
  const template = templates.get(page.template);
  if (!template) {
    throw new Error(`Missing website-md template: ${page.template}`);
  }
...
function sectionNavForPage(page) {
  ...
  if (page.slug === 'examples') return 'examples';
...
async function writeLanguageReferencePage(language) {
  if (language.apiKind === 'typedoc') {
    ...
    await writePage(`${language.id}/api/reference/_index.md`, {
      ...
      body_class: 'docs-api',
      toc: false,
      source: 'src/docs/src/content/apidocs',
      body,
    });
    return;
  }
...
async function writeInventoryPage(languages) {
  ...
  await writePage('_inventory/_index.md', {
    ...
    description: 'Source-audited website-md inventory.',
    weight: 9999,
    generated: true,
    ...
    toc: false,
    source: 'scripts/website-md-prepare.mjs',
    body: [
      '# Website Inventory',
      '',
...
async function readTypeDocPages() {
  const apiRoot = path.join(repoRoot, 'src/docs/src/content/apidocs');
  ...
  for (const file of files) {
    ...
    pages.push({
      ...
      source: `src/docs/src/content/apidocs/${rel}`,
    });
  }
...
function sourceForPage(language, page) {
  if (page.template === 'api') {
    return language.apiKind === 'typedoc'
      ? 'src/docs/src/content/apidocs'
      : `packages/${language.id}/axir-api.json`;
  }
  return `website-md/content-src/templates/${page.template}.md`;
}
...
async function copyMermaidVendor() {
  ...
  if (!(await exists(entry))) {
    throw new Error(
      'Missing Mermaid bundle. Run `npm install` before `npm run website-md:prepare`.'
    );
  }
...
async function writeRedirectPage(relPath, page) {
  await writePage(relPath, {
    ...
    source: 'website-md/content-src/site-map.json',
    body: [
      `# ${page.title}`,
      '',
