...
Grounding can also be made explicit: with `citations` enabled, the answer must cite which evidence entries support it, and the citations are validated against the ids that actually exist — an answer cannot point at evidence that was never collected. This behavior is shared by TypeScript and the generated Python, Java, C++, Go, and Rust packages.
