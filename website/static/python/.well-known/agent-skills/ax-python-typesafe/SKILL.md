...
```python
model = ai('typesafe', api_key=api_key, trueThreshold=0.9)
triage = ax('ticket:string -> urgent:boolean(true "Core task blocked", false "Routine request"), team:class "support, billing, engineering"')
...
Runnable signature, native criteria/scoring, and two-program hybrid examples are under src/examples/python/generation/. See https://axllm.dev/python/examples/generation/.
